...
class TestV03:
    ...
    def test_notification_callback_crud(self):
        notif = Notification.objects.create(test_job=self.finished_job)
        url = reverse("api-root", args=[self.version]) + "notification-callbacks/"
        ...
        # List (visible because finished_job is_public=True)
        r = self.user_c.get(url)
        assert r.status_code == 200
        results = r.json()["results"] if isinstance(r.json(), dict) else r.json()
        assert any(cb["id"] == cb_id for cb in results)

        # Delete
        r = self.user_c.delete(f"{url}{cb_id}/")
        assert r.status_code in (204, 200)
        assert not NotificationCallback.objects.filter(id=cb_id).exists()

    # ------------- me/settings -------------

    def test_me_settings_get_creates_extended_user(self):
        assert not ExtendedUser.objects.filter(user=self.user).exists()
        url = reverse("api-root", args=[self.version]) + "me/settings/"
        r = self.user_c.get(url)
        assert r.status_code == 200, r.content
        assert ExtendedUser.objects.filter(user=self.user).exists()
        body = r.json()
        assert "table_length" in body

    def test_me_settings_patch(self):
        url = reverse("api-root", args=[self.version]) + "me/settings/"
        r = self.user_c.patch(
            url, {"irc_handle": "alice", "table_length": 50}, format="json"
        )
        assert r.status_code == 200, r.content
        ext = ExtendedUser.objects.get(user=self.user)
        assert ext.irc_handle == "alice"
        assert ext.table_length == 50
