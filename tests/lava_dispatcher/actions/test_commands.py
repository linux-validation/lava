...
class TestCommands(LavaDispatcherTestCase):
            ...
            device_dict=PipelineDevice(
                {
                    "commands": {
                        ...
                        "users": {
                            ...
                            "secret_command": {"do": "echo mysecret!", "undo": "/bin/undo"},
    ...
    def test_command_create_secret(self):
        self.action.parameters = {"name": "secret_command", "create_secret": "TEST_CREATE_SECRET"}
        self.action.validate()
        self.action.run(None, 600)
        self.assertIsNotNone(self.job.secrets)
        self.assertEqual(self.job.secrets['TEST_CREATE_SECRET'], "mysecret!")
    ...
    def test_command_bad_command_with_secret(self):
        self.action.parameters = {"name": "bad_command", "create_secret": "BAD_COMMAND_SECRET"}
        self.action.validate()
        with self.assertRaises(InfrastructureError):
            self.action.run(None, 600)

    def test_command_secret_no_output(self):
        self.action.parameters = {"name": "empty_command", "create_secret": "EMPTY_SECRET"}
        self.action.validate()
        self.action.run(None, 600)
        self.assertNotIn("EMPTY_SECRET", self.job.secrets)
