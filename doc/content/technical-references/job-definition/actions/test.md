...
`strip-components` is an optional integer that removes the given number of
leading path components from each member when extracting, mirroring GNU tar's
`--strip-components`. This is useful for archives generated by services such as
GitHub and GitLab, whose "archive" tarballs wrap their contents in a top-level
directory (for example `test-definitions-2019.03/`). With `strip-components: 1`
that wrapping directory is removed so `path` can be given relative to the
repository root.
