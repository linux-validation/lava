...
class SchedulerWorkersAPI(ExposedV2API):
    ...
    def show(self, hostname):
        ...
        data = {
            ...
            # capacity as of last_ping; None when the worker did not report it
            "kernel": worker.kernel,
            "arch": worker.arch,
            "boot_time": worker.boot_time,
            "cpu_model": worker.cpu_model,
            "nproc": worker.nproc,
            "load_1": worker.load_1,
            "load_5": worker.load_5,
            "load_15": worker.load_15,
            "tmp_dir": worker.tmp_dir,
            "tmp_disk_total": worker.tmp_disk_total,
            "tmp_disk_free": worker.tmp_disk_free,
            "mem_total": worker.mem_total,
            "mem_available": worker.mem_available,
